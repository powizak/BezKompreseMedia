...
- **Data layer**: each service page has a typed `.ts` file in `src/data/` exporting content objects. `src/data/contact.ts` exports `siteContact` — the single source of truth for phone/email used by SiteFooter, PageCTA, QuickContact, and the contact page. `src/data/schema.ts` exports JSON-LD structured data (Organization on every page, WebSite + ItemList on home, per-page Service/BreadcrumbList/OfferCatalog) rendered by BaseLayout's `schema` prop — when changing prices in `cenik.ts` or contact details, update `schema.ts` to match. No CMS, no API calls.
...
- **Client JS**: `src/scripts/scroll-animations.js` (IntersectionObserver, `.scroll-reveal`/`.visible`, `data-delay`/`data-threshold`). `src/scripts/webmcp.ts` registers WebMCP tools (navigate/get_contact_info/get_pricing/get_page_info/start_inquiry) for in-browser AI agents — progressive enhancement, no-op without `document.modelContext`; tool data must come from `src/data/*.ts` imports (no duplication); gated by the `tools` Permissions-Policy set in `public/.htaccess`.
- **Server-side MCP**: `public/mcp.php` is a stateless MCP server (Streamable HTTP, JSON-RPC 2.0) exposed at `/mcp` (rewrite in `.htaccess`), discovered via `/.well-known/mcp.json` and `<link rel="mcp">` in BaseLayout. Tools: get_services/get_pricing/get_contact_info/get_faq (read `dist/mcp-data.json`) + submit_inquiry (mail pipeline shared with contact-handler.php, file-based per-IP rate limit 3/h). `src/pages/mcp-data.json.ts` exports site data at build time from the same `src/data/*.ts` modules — when changing cenik/contact/faq data, the export regenerates automatically on build; keep `mcp.php` tool schemas in sync with data shapes manually.
